...
/*< forward Kirchhoff time migration of 1 model trace to all data traces >*/
{
  ...
  float ang,geoms,obliq,angtaper;
  ...
  for (icmpx=0;icmpx<ncmpx;icmpx++){
    ...
    for (it=1;it<nt;it++){
	  ...
	  geoms=sqrtf(1/(t*vp[icipx][it]));
	  obliq=sqrtf(.5*(1 + (t0*t0/(4*ts*tg))- (1/(ts*tg))*sqrtf(ts*ts - t0*t0/4)*sqrtf(tg*tg - t0*t0/4)));
	  ang=180.0*fabs(acosf(t0/t))/PI;  
	  if(ang<=40) angtaper=1.0;
	  if(ang>40) angtaper=cosf((ang-40)*PI/20);
      w=geoms*obliq*angtaper;

      t_floor = dt*(truncf((t-ot)/dt) + 1) + ot;
      jt = (int) truncf((t-ot)/dt) + 1;
      if (jt >= 0 && jt+1 < nt){
	    res = (t-t_floor)/dt;
	    res0 = 1.0-res;
